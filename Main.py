...
cols = getTickers()
startdate = st.sidebar.date_input("Start Date", datetime.date(2021, 6, 9)) # '2022-01-01'
ticker = st.sidebar.selectbox('Select a Stock', cols, index=cols.index("AAPL"))

df = retrieveTicker(ticker,startdate)
# st.table(Chase[:10])

tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["matplotlib", "plotly", "altair", "vega-lite", "seaborn", "bokeh"])
...
with tab5:
    ...
    ax.title.set_text(ticker)
    sns.lineplot(data=stock, x="Date", y="Close", ax=ax)
    st.pyplot(figs)
with tab6:
